...
export const useLightning = (api: AxiosInstance): LightningInstance => {
  ...
  const getNodesInCountry = async (params: { country: string }) => {
    const { data } = await api.get<Node[]>(
      `/v1/lightning/nodes/country/${params.country}`,
    );
    return data;
  };
  ...
  const getNodesHostedByISP = async (params: { isp: number }) => {
    const { data } = await api.get<Node[]>(
      `/v1/lightning/nodes/isp/${params.isp}`,
    );
    return data;
  };
  ...
  const getNodeStats = async (params: { public_key: string }) => {
    const { data } = await api.get<Node>(
      `/v1/lightning/nodes/${params.public_key}`,
    );
    return data;
  };

  const getHistoricalNodeStats = async (params: { public_key: string }) => {
    const { data } = await api.get<NodeStats[]>(
      `/v1/lightning/nodes/${params.public_key}/statistics`,
    );
    return data;
  };
  ...
  const getChannel = async (params: { id: string }) => {
    const { data } = await api.get<Channel>(
      `/v1/lightning/channels/${params.id}`,
    );
    return data;
  };
  ...
  const getChannelsFromTxIds = async (params: { txId: string[] }) => {
    const { data } = await api.get<any[]>(
      `/v1/lightning/channels/txids?txId[]=${params.txId.join('&txId[]=')}`,
    );
    return data;
  };

  const getChannelsFromNodePublicKey = async (params: {
    public_key: string;
    status: string;
    index?: number;
  }) => {
    const { data } = await api.get<Channel[]>(
      `/v1/lightning/channels?public_key=${params.public_key}&status=${params.status}&index=${params?.index}`,
    );
    return data;
  };
  ...
  const getChannelsGeodataByPublicKey = async (params: {
    public_key: string;
  }) => {
    const { data } = await api.get<any[]>(
      `/v1/lightning/channels-geo/${params.public_key}`,
    );
    return data;
  };
